...
describe("editor store unsaved changes workflow", () => {
  ...
  it("switches imported material slots without dirtying the project", () => {
    useEditorStore.setState({
      selectedId: "imported-node:0",
      selectedIds: ["imported-node:0"],
      selectedName: "DualMatPanel",
    ...
    expect(useEditorStore.getState().selectedId).toBe("imported-node:0");
